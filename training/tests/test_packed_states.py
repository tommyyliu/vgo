...
class PackedStateTests(unittest.TestCase):
    ...
    def test_rejects_a_foreign_channel_count(self) -> None:
        # A width with no layout.
        unknown = max(_LAYOUTS) + 1
        states = torch.zeros((2, unknown, 4, 4), dtype=torch.float16)
        self.assertFalse(is_packable(states))

    def test_packs_the_radius_layout(self) -> None:
        """`compact-radius` packs: three bit planes, and pass and radius as scalars."""
        samples, height, width = 6, 8, 8
        states = torch.zeros((samples, 7, height, width), dtype=torch.float16)
        generator = torch.Generator().manual_seed(7)
        for channel in (0, 1, 3):  # stones, stones, settled
        ...
        for sample in range(samples):
            ...
            states[sample, 6] = 2.0 / 38.0              # radius

        self.assertTrue(is_packable(states))
        packed = pack(states)
        self.assertEqual(packed.channels, 7)
        self.assertEqual(packed.shape, (samples, 7, height, width))
        self.assertTrue(torch.equal(packed.expand(), states))
        self.assertEqual(packed.scalars.shape, (samples, 3))
