...
/// cores.
///
/// For this pipeline that default is exactly backwards. A profile of the
...
pub fn use_blocking_sync(device: usize) -> Result<(), CudaError> {
    ...
    unsafe {
        ...
        .result()
        .map_err(|e| CudaError::Unavailable(format!("set blocking sync: {e}")))?;
    }
    Ok(())
}
